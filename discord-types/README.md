...
While it was primarily created for Kernixcord based on Vencord and its respective packages, other client mods could also benefit from this, so it is published as a standalone package!

## Installation

```bash
npm install -D @kernixcord/discord-types
yarn add -D @kernixcord/discord-types
pnpm add -D @kernixcord/discord-types
```

## Example Usage

```ts
import type { UserStore } from "@kernixcord/discord-types";
...
```ts
import { ApplicationCommandType } from "@kernixcord/discord-types/enums";
