...
async function discoveredDiscordRoots(): Promise<string[]> {
    ...
    return entries
        .filter((entry: Dirent) => entry.isDirectory() && /(discord|cord|vencord|kernixcord)/i.test(entry.name))
...
function clientPrefixForRoot(root: string): string {
    const leaf = basename(root).toLowerCase();
    if (["discord", "vencord", "kernixcord", "bettervencord"].includes(leaf)) return "Stable";
...
function Stop-DiscordUnderRoot {
    ...
    $names = @("Discord.exe", "DiscordCanary.exe", "DiscordPTB.exe", "DiscordDevelopment.exe", "Lightcord.exe", "Vencord.exe", "Kernixcord.exe", "BetterVencord.exe", "Update.exe")
